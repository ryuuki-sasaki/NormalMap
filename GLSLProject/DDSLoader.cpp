...
void DDSImage::DecompressDDS()
{
	...
	for (int i = 0; i < (int)numMipmaps; i++)
	{
		...
		glCompressedTexImage2D(GL_TEXTURE_2D, i, format, mWidth, mHeight, 0, mSize, imageData + offset);	//圧縮データをテクスチャオブジェクトに読み込む
...
GLuint DDSImage::Load(const char *filename)
{
	...
	//テクスチャオブジェクトを生成して、そのハンドルをIDに格納
	glGenTextures(1, &ID);

	//テクスチャオブジェクトをGL_TEXTURE_2Dにバインド
